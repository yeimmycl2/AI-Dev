...
function formatToolCallMessage(toolName: string, args: any): string {
            ...
            <div className={cn(
              ...
              <div className={cn(
                ...
                message.role === "user"
                  ...
                  : "bg-gradient-to-br from-gray-50 to-blue-50 text-neutral-900 border-2 border-blue-200 shadow-md"
              )}>
                          ...
                          case "tool-invocation":
                            ...
                            return (
                              <div key={partIndex} className={cn(
                                "inline-flex items-center gap-2 mt-2 px-3 py-1.5 rounded-lg text-xs border-2 shadow-md transition-all",
                                tool.state === "result" && tool.result
                                  ? "bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-300 shadow-emerald-200/50"
                                  : "bg-gradient-to-r from-blue-50 to-purple-50 border-purple-300 shadow-purple-200/50 animate-pulse"
                              )}>
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <div className="w-2 h-2 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 shadow-md shadow-emerald-400/50"></div>
                                    <span className="text-emerald-800 font-medium">{formatToolCallMessage(tool.toolName, tool.args)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-purple-600" />
                                    <span className="text-purple-700 font-medium">{formatToolCallMessage(tool.toolName, tool.args)}</span>
                                  </>
                                )}
                              </div>
                            );
